...
function App() {
    ...
    progressPollRef.current = setInterval(async () => {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/attendance/progress`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  useEffect(() => {
    ...
    const shownCount = parseInt(
      localStorage.getItem(SWIPE_HINT_STORAGE_KEY) || '0',
      10
    );
  ...
  const fetchAttendance = async () => {
    ...
    setFetchProgress({
      step: 0,
      total: 8,
      message: 'Connecting to OLT portal…',
      status: 'in_progress'
    });
    ...
    try {
      ...
      setAttendanceFetchedSection(
        res.data.section || sectionAtFetchTime
      );
    } catch (err) {
      setAttendanceError(
        err.response?.data?.error ||
        'Failed to fetch attendance from OLT.'
      );

      if (err.response?.data?.needsCredsReset) {
  ...
  const saveCredentials = async (username, password) => {
    try {
      ...
      await axios.post(
        `${API_BASE_URL}/api/user/olt-credentials`,
        { username, password },
        {
          headers: {
            Authorization: `Bearer ${token}`
      ...
      alert('Failed to securely save credentials.');
    }
  };
  ...
  const currentDayData = scheduleData.find(
  ...
  const minutesToNextRefresh = () => {
    ...
    const diffMs =
      syncMeta.nextRefreshTime - nowTick;
  ...
  const goToDay = (direction) => {
    ...
    const newDate =
      shiftIsoDate(selectedDate, delta);
  ...
  const injectedStyles = `
    ...
    @media (max-width: 768px) {
      ...
      .main-content {
        padding-bottom: ${hasTasksToday ? '160px' : '120px'} !important;
      ...
      .mobile-bottom-nav {
        ...
        border-radius: 30px;
        box-shadow:
          0 18px 45px rgba(10, 16, 28, 0.28),
          0 4px 14px rgba(10, 16, 28, 0.18);
        z-index: 1200;
        box-sizing: border-box;
      ...
      .mobile-bottom-nav-btn.active {
        ...
        box-shadow: 0 5px 18px rgba(240, 200, 92, 0.2);
      }
      ...
      .mobile-bottom-nav-btn.active:active {
      ...
      .mobile-bottom-nav-btn.active .mobile-bottom-nav-badge {
        border-color: #f0c85c;
      }
    ...
    .swipe-left {
      animation: premiumSlideInRight 0.55s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }

    .swipe-right {
      animation: premiumSlideInLeft 0.55s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    ...
    /* --- OLT POPUP GLOW --- */
    .olt-glow-modal {
      background: linear-gradient(to bottom, #fff, #fefdf9);
      border: 1px solid var(--accent-gold);
      box-shadow: 0 0 20px rgba(219, 163, 21, 0.2);
      text-align: center;
    }
    ...
    .feature-banner {
      background: linear-gradient(
        135deg,
        rgba(219, 163, 21, 0.12),
        rgba(219, 163, 21, 0.04)
      );
      border: 1px solid rgba(219, 163, 21, 0.25);
      border-radius: 12px;
      padding: 1.25rem;
    ...
    .session-badge {
      ...
      color: var(--accent-gold);
      background: rgba(219, 163, 21, 0.12);
    ...
    .add-task-btn {
      background: rgba(219, 163, 21, 0.1);
      border: 1px solid rgba(219, 163, 21, 0.3);
      border-radius: 8px;
      padding: 6px;
      display: flex;
    ...
    .todo-input-form button:disabled {
      background: #e0c88b;
      cursor: not-allowed;
    }
    ...
    .todo-summary-item:last-child {
      margin-bottom: 0;
    }

    .todo-summary-item.completed {
      opacity: 0.5;
      text-decoration: line-through;
    }
    ...
    .active-user-time-val {
      font-size: 0.9rem;
      font-weight: 600;
      color: #334155;
    }

    .active-user-time-label {
      font-size: 0.75rem;
      color: #94a3b8;
      text-transform: uppercase;
      letter-spacing: 0.5px;
    ...
    /* --- LIVE ATTENDANCE FETCH PROGRESS --- */
    ...
    .fetch-progress-ring {
      ...
      background: conic-gradient(
        var(--accent-gold, #dba315) calc(var(--pct) * 1%),
        rgba(219, 163, 21, 0.12) 0
      );
      transition: background 0.6s cubic-bezier(0.4, 0, 0.2, 1);
    }

    .fetch-progress-ring::before {
      content: '';
      position: absolute;
      inset: 8px;
      border-radius: 50%;
    ...
    .fetch-progress-ring-label {
      position: relative;
      z-index: 1;
      font-size: 1.3rem;
      font-weight: 700;
      color: var(--accent-gold, #dba315);
      letter-spacing: 0.3px;
    }
    ...
    .fetch-progress-dot.done {
      background: var(--accent-gold, #dba315);
    }

    .fetch-progress-dot.current {
      background: var(--accent-gold, #dba315);
      animation: fetchDotPulse 1.1s ease-in-out infinite;
    }
      ...
      <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
        ...
        >
          ...
          >
            ...
            >
              *Requires @iimtrichy.ac.in email address
            </p>
          </div>
        </div>
      </GoogleOAuthProvider>
  ...
  const renderAttendanceTab = () => {
    ...
    if (isFetchingAttendance) {
      ...
      return (
        <div className="attendance-fetch-progress">
          <div
            className="fetch-progress-ring"
            style={{ '--pct': pct }}
          >
            <span className="fetch-progress-ring-label">
              {pct}%
            </span>
          </div>

          <div className="fetch-progress-message">
            {fetchProgress.message ||
              'Connecting to OLT Portal…'}
          </div>
    ...
    const noRecordsMatched =
      subjectCount === 0 ||
      overallTotal === 0;

    const sectionMismatch =
      attendanceFetchedSection &&
      attendanceFetchedSection !== section;
        ...
        <div className="attendance-container">
          <div className="creds-card mismatch-card">
            ...
            {sectionMismatch ? (
              ...
              >
                We fetched attendance for{' '}
                <strong>
                  Section {attendanceFetchedSection}
                </strong>
                , but you're currently browsing{' '}
                <strong>
                  Section {section}
                </strong>
                . Attendance is always fetched for your actual class section. If that's still{' '}
                {attendanceFetchedSection}, switch the timetable section back below. Otherwise, your OLT credentials may need updating.
              </p>
            ) : (
              <p
                style={{
                  ...
                  {data.classes.map(
                    (cls, cIdx) => (
                      ...
                      >
                        ...
                        <span
                          className={`status-badge status-${cls.status.toLowerCase()}`}
                        >
                          {cls.status}
                        </span>
                      </div>
          ...
          <div className="modal-content olt-glow-modal">
            ...
            >
              <button
                className="btn-submit"
                onClick={() => {
                  setShowOltPopup(false);
                  handleTabChange('attendance');
                }}
              >
                Setup OLT Account Now
              </button>
      ...
      {showSettingsModal && (
        ...
        >
          ...
          >
            ...
            >
              Choose your section. This is saved to your account and remembered next time you log in.
            </p>
              ...
              {SECTIONS.map(sec => (
                <button
                  ...
                  className={`settings-section-btn ${
                    settingsSectionDraft === sec
                      ? 'active'
                      : ''
                  }`}
                  onClick={() =>
                    setSettingsSectionDraft(sec)
                  }
                >
                  {sec}
                </button>
        ...
        >
          <div className="swipe-tutorial-card">
            <div className="swipe-tutorial-track">
              <Hand
                size={40}
                className="swipe-tutorial-hand"
                strokeWidth={1.75}
              />
            </div>

            <div className="swipe-tutorial-text">
              Swipe left or right to change day
            </div>

            <div className="swipe-tutorial-dismiss">
                ...
                <>
                  <div className="live-data-banner">
                    ...
                    <span>
                      ...
                      synced directly from Excel, no need to refresh manually.
                    </span>
...
function CredentialForm({
  ...
  return (
    <div className="creds-card">
      ...
      <h3>
        {hasCreds ? 'Update' : 'Link'} OLT Account
      </h3>
      ...
      >
        Your credentials are encrypted and stored securely to sync your attendance.
      </p>
...
//       if (err.response?.data?.needsCredsReset) setShowCredsForm(true);
//     } finally {
//       setIsFetchingAttendance(false);
//       stopProgressPolling();
//     }
